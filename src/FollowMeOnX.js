...
const FollowMeOnX = () => {
  const handleClick = () => {
    window.open('https://x.com/xetur_xyz', '_blank');
  };
    ...
    >
      ...
      >
        <div style={{ background: '#343132', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px', border: '1px solid #ccc', borderRadius: '10px', width: '250px', margin: '0 auto' }}>
          ...
          />
          <span style={{ paddingTop: '16px', fontSize: '16px', color: 'white', fontWeight: 'bolder' }}>@xetur_xyz</span> {/* Black color for the handle */}
        </div>
      </div>
    </div>
  );
